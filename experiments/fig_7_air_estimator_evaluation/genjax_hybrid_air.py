...
v = tfp.distributions.MultivariateNormalDiag(
    loc=img[0, :, :], scale_diag=0.03 * jnp.ones_like(img[0, :, :])
).sample(seed=key)
show_images([v], "./img/noised_multimnist_sample.pdf")
...
def guide_step(
    ...
    if t < 2:
        ...
        z_pres = (
            vi.flip_enum((eps + (z_pres_p[0] * prev_z_pres[0])) / (1 + 1.01 * eps))
            @ f"z_pres_{t}"
        )
    z_pres = jnp.array([z_pres.astype(int)])
...
for train_idx in range(0, 5):
    ...
    df.to_csv(
        f"./training_runs/vi_air_hybrid_mvd_enum_epochs_21_{train_idx}.csv",
        index=False,
    )
    if losses is None:
        losses = r_loss
...
df.to_csv("./training_runs/vi_air_hybrid_mvd_enum_epochs_21.csv", index=False)


key = jax.random.PRNGKey(314159)
v = tfp.distributions.MultivariateNormalDiag(jnp.zeros(50), jnp.ones(50)).sample(
    seed=key
)
v = params[0](v)
show_images([v], "./img/trained_decoder_sample")
...
for train_idx in range(0, 5):
    key, sub_key = jax.random.split(key)
    r_loss, r_acc, r_times, params = train(
        sub_key, learning_rate=1.0e-4, n=2, batch_size=64, num_epochs=20
    )
    # Save run.
    arr = np.array([r_loss, r_acc, r_times])
    df = pd.DataFrame(
    ...
    df.to_csv(
        f"./training_runs/vi_air_iwae_2_hybrid_mvd_enum_epochs_21_{train_idx}.csv",
        index=False,
    )
    if losses is None:
        losses = r_loss
...
df.to_csv(
    "./training_runs/genjax_air_iwae_2_hybrid_mvd_enum_epochs_21.csv",
    index=False,
)
