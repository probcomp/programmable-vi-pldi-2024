...
def train(key, n=1, num_epochs=40, batch_size=64, learning_rate=1.0e-3):
    def svi_update(model, guide, optimiser):
        def batch_updater(key, params, opt_state, data_batch):
            def p_grads(key, params, data):
                ...
                objective = vi.p_wake(
                    model, (decoder,), guide, (data, rnn, encoder, predict), data, n
                )
            ...
            p_loss, (decoder_grads,) = jax.vmap(p_grads, in_axes=(0, None, 0))(
            ...
            grads = jtu.tree_map(lambda v: 1.0 * jnp.mean(v, axis=0), grads)
    ...
    for i in range(0, num_epochs + 1):
        ...
        params, opt_state, (p_loss, q_loss) = epoch_train(
            opt_state, params, sub_key, train_idx
        )
